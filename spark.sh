...
export DOCKER_IMAGE=shuvamoy008/sparkk8s
...
export ElastiClusterIP=`kubectl get svc -n elk | grep -i elasticsearch-master-0 | awk '{print $4}'`
export EIP=`curl http://$ElastiClusterIP:9200/_cluster/state?pretty | grep -i '9300' | grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}'`
echo "EIP IS $EIP"
...
kubectl exec -it deployment.apps/spark-master -n $SPARK_NAMESPACE -- /bin/bash -c "/opt/spark/bin/spark-submit --name sparkkafka-test123 \
  ...
  local:///opt/spark/examples/jars/GCP-1.0-SNAPSHOT-jar-with-dependencies.jar $KafkaBootstrapIP $cassandraHost $cassandraUserName $cassandraPassword $mySQLIP $MySqlUserName $MySqlPassword $EIP"
