...
object ConnectSession {
  ...
  private def createSparkSession(appName: String,cassandraHost:String,cassandraUserName:String,cassandraPassword:String,elasticsearchIP:String): SparkSession = {
    ...
    val session = SparkSession.builder().appName(appName).config(conf).config("spark.es.nodes.discovery", "false").config("spark.es.index.auto.create","true").config("spark.es.nodes.discovery", "false").config("spark.es.nodes",elasticsearchIP).config("spark.es.port","9200").config("es.nodes.wan.only","true").getOrCreate()
