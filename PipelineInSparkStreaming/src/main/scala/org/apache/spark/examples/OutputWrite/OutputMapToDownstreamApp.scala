...
object OutputMapToDownstreamApp {
  ...
  def writeKafkaToMultipleApp(topicName : ListBuffer[String], schemaName: ListBuffer[String], KafkaIP:String,config: Configurations,KibanaTransportIp: String) = {
              ...
              for(a <- 0 until 1 ){
              {
                val thread = new Thread {
                  override def run: Unit = {
                    ...
                    if (!(config.outputOptions.apply("KibanaIndex") == null)) org.apache.spark.examples.OutputWrite.WriteToKibana.writeTOKibanaIndex(topicDF,KibanaTransportIp,getPartialTopicName)
